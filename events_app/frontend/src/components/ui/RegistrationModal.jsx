...
export default function RegistrationModal({event, closeModal}) {
    ...
    const handleFormAction = async (e) => {
        ...
        try {
            ...
            if (serverErrors.error && typeof serverErrors.error === 'string') {
                safeErrorState.general = serverErrors.error
            } else if (serverErrors.detail) {
                safeErrorState.general = serverErrors.detail
            } else if (serverErrors.non_field_errors && Array.isArray(serverErrors.non_field_errors)) {
                safeErrorState.general = serverErrors.non_field_errors[0]
            } else {
    ...
    const initiateRazorpayCheckout = async (orderData) => {
        ...
        const options = {
            ...
            handler : async function (response) {
                try {
                    ...
                    const verifyRes = await api.post('/api/events/verify-payment/', {
                    ...
                    if (verifyRes.status === 200) {
                        toast.success("Payment successful! Tickets generated.", {id : 'payment-verify'})
                    ...
                    toast.error("Verification failed. Contact support if money was deducted.", {id : 'payment-verify'})
            ...
            modal : {
                ondismiss : async function() {
                    ...
                    try {
                        await api.post('/api/events/cancel-pending-order/', {
                            razorpay_order_id : orderData.razorpay_order_id
                        })
                    } catch (err) {
                        console.error("Failed to fast-release tickets. Celery will catch it.", err)
                    }
                }
            }
        }
        ...
        paymentObject.on('payment.ready', function() {
            setLoading(false)
        })
    ...
    return (
        ...
        >
            ...
            {/* Modal Container */}
            <div className = "relative max-h-[95vh] w-full sm:max-w-xl overflow-y-auto bg-zinc-950 border-t border-zinc-800 sm:border p-8 rounded-t-3xl sm:rounded-3xl shadow-2xl animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-300">
                {/* Close button */}
                <button
                    onClick = {closeModal}
                    disabled = {loading}
                    className = "absolute top-4 right-4 z-10 p-2 rounded-full bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white transition-colors disabled:opacity-50"
                >
                    <X className = "h-5 w-5" /> 
                </button>
                        ...
                        >
                            ...
                            <div className = "mb-8 p-4 bg-zinc-900/50 rounded-2xl border border-zinc-800 flex items-center justify-between">
                                ...
                                <div className = "flex items-center gap-4 bg-black rounded-lg p-1 border border-zinc-800">
                                    <button
                                        ...
                                        disabled = {loading}
                                        className = "p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-md disabled:opacity-30 disabled:hover:bg-transparent disabled:cursor-not-allowed transition-all"
                                    >
                            ...
                            <div className = "mt-8 pt-4 border-t border-zinc-900">
                                <button
                                    type = 'submit'
                                    disabled = {loading}
                                    className = "group relative w-full flex overflow-hidden rounded-xl bg-white py-3 text-base font-bold text-black shadow-lg transition-transform active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {loading ? (
