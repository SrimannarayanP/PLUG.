...
import {
    Archive, Ban, CalendarDays, ChartNoAxesColumnDecreasing, CheckCircle, ChevronLeft, ChevronRight, ExternalLink, Loader2, MapPin, RefreshCw, Send, User, X
} from 'lucide-react'
...
export default function TicketModal({tickets : initialTickets, closeModal, onTicketChange}) {
    ...
    const timeUntilEvent = new Date(event.start_date).getTime() - new Date().getTime()
    const canCancel = timeUntilEvent > (24 * 60 * 60 * 1000)
    ...
    return createPortal(
        ...
        <>
                                    ...
                                    {!isCancelled && !currentTicket.is_checked_in && !isRefundPending && !isRefundProcessed && !isExpired && (
                                        <div className = "flex flex-col gap-1.5">
                                            ...
                                            >
                                                ...
                                                {cancelling
                                                    ? 'Cancelling...'
                                                    : (canCancel ? "Cancel Ticket" : "Cancellation Closed")}
                                            </button>

                                            {!canCancel && (
                                                <p className = "text-[9px] text-zinc-500 text-center uppercase tracking-wider font-bold">
                                                    Locked within 24 hours of event start
                                                </p>
                                            )}
                                        </div>
                                    )}
            ...
            <ConfirmDialog
                ...
                title = "Cancel Ticket"
                message = "Are you sure you want to cancel this ticket? If the payment was already verified, a refund will be initiated. This action cannot be undone."
                isDestructive = {true}
