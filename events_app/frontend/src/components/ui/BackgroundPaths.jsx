...
function FloatingPaths({position}) {
    const paths = Array.from({length : 36}, (_, i) => ({
        id : i,
        d : `M-${380 - i * 5 * position} -${189 + i * 6}C-${380 - i * 5 * position} -${189 + i * 6} -${312 - i * 5 * position} ${216 - i * 6} ${152 - i * 5 * position}
    ...
    return (
        ...
        <div className = "absolute inset-0 pointer-events-none">
            <svg 
                className = "w-full h-full text-yellow-400 drop-shadow-[0_0_15px_rgba(250, 204, 21, 0.4)]" 
                viewBox = "0 0 696 316" 
                fill = 'none'
            >
                {paths.map((path) => (
