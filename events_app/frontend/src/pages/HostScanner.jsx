...
export default function HostScanner() {
    ...
    const handleManualSubmit = (e) => {
        ...
        if (manualCode.trim().length > 0) processTicket({ticket_code : manualCode, event_id : eventId})
    }
    ...
    return (
        ...
        <div className = {`fixed inset-0 z-[9999] w-full overflow-hidden flex flex-col ${getThemeColor()} transition-colors duration-300 transform-gpu`}>
            <div className = "flex-1 relative bg-black">
                <div className = "absolute top-0 left-0 right-0 p-4 sm:p-6 pt-6 sm:pt-8 flex justify-between items-center z-30 bg-gradient-to-b from-black/80 to-transparent pointer-events-none transform-gpu">
                    {/* Header */}
                    <button
                        onClick = {() => navigate('/host/dashboard')}
                        className = "pointer-events-auto p-3 bg-black/80 hover:bg-black rounded-full text-white active:scale-95 transition-transform border border-white/20 transform-gpu"
                    >
                        <ArrowLeft className = "h-5 w-5" />
                    </button>

                    <div className = "text-white font-mono text-xs md:text-sm font-bold tracking-widest opacity-80 uppercase bg-black/80 px-4 py-2 rounded-full border border-white/10 transform-gpu">
                    ...
                    <button
                        ...
                        className = {`
                            pointer-events-auto p-3 rounded-full transition-all active:scale-95 border transform-gpu
                            ${torchEnabled
                    ...
                    <>
                        <div className = "absolute inset-0 z-0">
                            <Scanner 
                                ...
                                allowMultiple = {false}
                                scanDelay = {2000}
                                constraints = {{facingMode : 'environment', advanced : [{torch : torchEnabled}]}}
                                styles = {{
                                    container : {
                        ...
                        <div className = "absolute inset-0 flex items-center justify-center z-10 pointer-events-none transform-gpu">
                            <div className = "absolute inset-0 shadow-[0_0_0_9999px_rgba(0, 0, 0, 0.6)]" />

                            <div className = "h-64 w-64 border-2 border-white/20 rounded-xl relative overflow-hidden transform-gpu">
                                ...
                                <div className = "absolute top-0 left-0 h-0.5 w-full bg-orange-500 shadow-[0_0_8px_2px_rgba(249, 115, 22, 0.8)] animate-[scan_2s_ease-in-out_infinite] transform-gpu will-change-transform" />
                            </div>
                        </div>

                        {/* Manual Entry Trigger */}
                        <div className = "absolute bottom-0 left-0 right-0 p-8 pb-12 flex justify-center z-20 bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none transform-gpu">
                            <button
                                onClick = {() => setManualEntryOpen(true)}
                                className = "pointer-events-auto flex items-center gap-3 px-8 py-4 bg-zinc-900/90 backdrop-blur-md border border-zinc-700/50 rounded-full text-white font-bold text-sm uppercase tracking-widest shadow-[0_8px_30px_rgba(0, 0, 0, 0.5)] hover:bg-zinc-800 active:scale-95 transition-all transform-gpu"
                            >
                ...
                {!isScanning && (
                    <div className = "absolute inset-0 z-30 bg-black/90 flex flex-col items-center justify-center p-6 animate-in fade-in zoom-in duration-200 transform-gpu">
                        {scanStatus === 'loading' && <Loader2 className = "h-16 w-16 text-white animate-spin mb-4 transform-gpu" />}
                        ...
                        {attendeeInfo && (
                            <div className = "bg-white w-full max-w-sm rounded-xl p-5 shadow-2xl mt-6 text-zinc-900 transform transition-all transform-gpu">
                        ...
                        <button
                            ...
                            className = "mt-8 w-full max-w-xs py-4 bg-white text-black font-black text-lg uppercase tracking-widest rounded-xl shadow-xl active:scale-95 transition-transform hover:bg-gray-100 transform-gpu"
                        >
                            Scan Next
                        </button>
                    </div>
            ...
            {manualEntryOpen && (
                <div className = "absolute inset-0 z-50 bg-black/95 flex items-center justify-center p-4 transform-gpu">
                    <div className = "bg-[#18181b] w-full max-w-sm rounded-2xl border border-zinc-800 p-6 shadow-2xl animate-in slide-in-from-bottom-10 transform-gpu">
                        ...
                        <form onSubmit = {handleManualSubmit}>
                            <div className = 'space-y-4'>
                                ...
                                <button
                                    type = 'submit'
                                    disabled = {!manualCode}
                                    className = "w-full py-3 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-xl uppercase tracking-widest transition-colors disabled:opacity-50 disabled:cursor-not-allowed transform-gpu"
                                >
                                    Verify Code
                                </button>
                            </div>
